...
spark.sql("select product.name as pro_product,product.price as pro_price, supplier.name as sup_name from product inner join supplier on product.supplierid = supplier.supplierid").show()

spark.sql("select supplier.name as sup_name from product inner join supplier on product.supplierid = supplier.supplierid where product.name = 'Pencil 3B'").show()